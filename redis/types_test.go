...
func TestRedisDataStructure_Get(t *testing.T) {
	...
	assert.Equal(t, hades.ErrKeyNotFound, err)
}

func TestRedisDataStructure_Del_Type(t *testing.T) {
	settings.Init("../hades.yaml")
	opts := settings.Conf.DBConfig
	dir, _ := os.MkdirTemp("", "hades-del")
	opts.DirPath = dir
	rds, err := NewRedisDataStructure(opts)
	assert.Nil(t, err)
...
func TestRedisDataStructure_SIsMember(t *testing.T) {
	settings.Init("../hades.yaml")
	opts := settings.Conf.DBConfig
	dir, _ := os.MkdirTemp("", "hades-sisMember")
	opts.DirPath = dir
	rds, err := NewRedisDataStructure(opts)
	assert.Nil(t, err)
...
func TestRedisDataStructure_ZScore(t *testing.T) {
	settings.Init("../hades.yaml")
	opts := settings.Conf.DBConfig
	dir, _ := os.MkdirTemp("", "hades-zscore")
	opts.DirPath = dir
	rds, err := NewRedisDataStructure(opts)
	assert.Nil(t, err)
