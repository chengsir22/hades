...
func NewRedisDataStructure(options *settings.DBConfig) (*RedisDataStructure, error) {
	db, err := hades.Open(options)
	if err != nil {
		return nil, err
	}
...
// ======================= Hash 数据结构 =======================
// key---元数据          key|version|filed------value
...
func (rds *RedisDataStructure) HSet(key, field, value []byte) (bool, error) {
	...
	// 先查找是否存在
	var exist = true
	if _, err = rds.db.Get(encKey); err == hades.ErrKeyNotFound {
		exist = false
	}

	wb := rds.db.NewWriteBatch(settings.Conf.WriteBatchConfig)
	// 不存在则更新元数据
	if !exist {
		meta.size++
...
// ======================= Set 数据结构 =======================
// key---元数据          key|version|member------NULL
...
func (rds *RedisDataStructure) SAdd(key, member []byte) (bool, error) {
	...
	var ok bool
	if _, err = rds.db.Get(sk.encode()); err == hades.ErrKeyNotFound {
		// 不存在的话则更新
		wb := rds.db.NewWriteBatch(settings.Conf.WriteBatchConfig)
		meta.size++
		_ = wb.Put(key, meta.encode())
