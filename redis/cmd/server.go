...
func main() {
	...
	// 打开 Redis 数据结构服务
	redisDataStructure, err := redis.NewRedisDataStructure(settings.Conf.DBConfig)
	if err != nil {
		panic(err)
	}
...
func (svr *BitcaskServer) listen() {
	log.Println("bitcask cmd running, ready to accept connections.")
