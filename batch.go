...
type WriteBatch struct {
	options       *settings.WriteBatchConfig
	mu            *sync.Mutex
	db            *DB
...
func (db *DB) NewWriteBatch(opts *settings.WriteBatchConfig) *WriteBatch {
	if db.options.IndexType == settings.BPlusTree && !db.seqNoFileExists && !db.isInitial {
...
// Put 批量写数据 空key直接pass，other写入暂存区
...
// Delete 删除数据 空key直接pass； 索引数据不存在，查找暂存区，有则删掉否则直接返回； 索引中存在则将删除语句写入暂存区
...
// Commit 提交事务（必须串行化），将暂存的数据写到数据文件，并更新内存索引
