...
func Open(options *settings.DBConfig) (*DB, error) {
	// 对用户传入的配置项进行校验
	...
	//B+树索引不需要从数据文件中加载索引
	if options.IndexType != settings.BPlusTree {
		// 从 hint 索引文件中加载索引
		...
		if db.activeFile != nil {
			size, err := db.activeFile.IoSelector.Size()
			if err != nil {
				return nil, err
			}
...
func (db *DB) setActiveDataFile() error {
	...
	// 打开新的数据文件
	dataFile, err := data.OpenDataFile(db.options.DirPath, initialFileId, ioselector.StandardFIO)
	if err != nil {
		return err
	}
...
func (db *DB) loadDataFiles() error {
	...
	for i, fid := range fileIds {
		ioType := ioselector.StandardFIO
		if db.options.MMapAtStartup {
			ioType = ioselector.MemoryMap
		}
...
func checkOptions(options *settings.DBConfig) error {
	if options.DirPath == "" {
...
func (db *DB) resetIoType() error {
	...
	if err := db.activeFile.SetIOSelector(db.options.DirPath, ioselector.StandardFIO); err != nil {
		return err
	}
	for _, dataFile := range db.olderFiles {
		if err := dataFile.SetIOSelector(db.options.DirPath, ioselector.StandardFIO); err != nil {
			return err
		}
	}
	return nil
