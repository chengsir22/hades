...
func TestDB_FileLock(t *testing.T) {
	settings.Init("./hades.yaml")
	opts := settings.Conf.DBConfig
	dir, _ := os.MkdirTemp("./", "hades-FileLock")
	opts.DirPath = dir
	db, err := Open(opts)
	defer destroyDB(db)
	assert.Nil(t, err)
...
func TestDB_Backup(t *testing.T) {
	settings.Init("./hades.yaml")
	opts := settings.Conf.DBConfig
	dir, _ := os.MkdirTemp("", "hades-backup")
	opts.DirPath = dir
	db, err := Open(opts)
	defer destroyDB(db)
	assert.Nil(t, err)
